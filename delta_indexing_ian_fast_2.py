...
Filtered version: Only indexes images starting with "2"
"""
...
def find_delta_images(existing_bases: Set[str], ian_bases: Dict[str, Path]) -> List[Path]:
    """Find truly new images not in the existing index - FILTERED FOR 2"""
    
    logger.info("\n🔍 SMART DELTA ANALYSIS (FILTERED: 2)")
    logger.info("=" * 50)
    ...
    # Convert to paths and FILTER for images starting with 2
    all_delta_paths = [ian_bases[base] for base in new_bases]
    delta_paths = []
    
    for path in all_delta_paths:
        # Check if filename starts with 2
        if path.name[0] == '2':
    ...
    logger.info(f"   🔢 Images starting with 2: {len(delta_paths)}")
    logger.info(f"   ✅ Already indexed (different suffix): {len(already_indexed_bases)}")
    logger.info(f"   📈 Total in IAN/resized: {len(ian_bases)}")
    logger.info(f"   📊 Percentage filtered (2): {len(delta_paths)/len(all_delta_paths)*100:.1f}%" if all_delta_paths else "N/A")
...
def fast_index_delta_images(delta_paths: List[Path], checkpoint: str = "1095") -> tuple:
    ...
    logger.info(f"\n🔥 FAST INDEXING {len(delta_paths)} delta images on GPU 6...")
    logger.info("🔥" * 30)
    ...
    # Create LoRA engine
    logger.info("📥 Loading LoRA model for fast indexing on GPU 6...")
...
def merge_indexes(existing_index_path: str, delta_index, delta_embeddings, delta_paths) -> str:
    ...
    merged_base = f"indexes/v11_1095_db_pictures_512_merged_2_{timestamp}"
...
def main():
    """Main delta indexing workflow with fast performance - FILTERED FOR 2"""
    
    logger.info("🚀 FAST DELTA INDEXING FOR IAN/RESIZED (FILTERED: 2) - GPU 6")
    logger.info("=" * 60)
    logger.info("🎮 Using GPU 6 for processing")
    ...
    # Step 3: Find truly new images (filtered for 2)
    delta_paths = find_delta_images(existing_bases, ian_bases)
    
    if not delta_paths:
        logger.info("✅ No images starting with 2 need indexing!")
        return
    
    # Ask for confirmation
    logger.info(f"\n⚠️  About to process {len(delta_paths)} images starting with 2")
    ...
    logger.info(f"\n🎉 FAST DELTA INDEXING COMPLETE (FILTERED: 2)!")
    logger.info("=" * 40)
