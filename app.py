...
app.mount("/db_pictures_512", StaticFiles(directory=os.path.join(app_dir, "db_pictures_512"), follow_symlink=True), name="db_pictures_512")
...
# Session storage for batch results
BATCH_SESSIONS = {}  # session_id -> batch_results

def generate_session_id():
    """Generate a unique session ID"""
    return str(uuid.uuid4())

def store_batch_results(session_id, results, metadata=None):
    """Store batch results in session storage"""
    ...
    if len(BATCH_SESSIONS) > 10:
        oldest_session = min(BATCH_SESSIONS.keys(), key=lambda k: BATCH_SESSIONS[k]['timestamp'])
        del BATCH_SESSIONS[oldest_session]
    
    return session_id

def get_batch_results(session_id):
    """Retrieve batch results from session storage"""
    return BATCH_SESSIONS.get(session_id)
...
def get_image_path(filename_root):
    ...
    if hasattr(data_loader, 'get_image_path'):
        ...
        if path and os.path.exists(path):
            ...
            return f"/db_pictures_512/{os.path.basename(path)}"
    
    # Fallback to direct search in db_pictures_512 directory
    # db_pictures_512 uses just filename_root.jpg (no _O00 suffix)
...
async def home(request: Request):
    ...
    return templates.TemplateResponse("index_modern.html", {
        "request": request,
...
async def modern_interface(request: Request):
    """Modern interface (explicit route)"""
    filter_options = search_engine.get_filter_options() if INITIALIZATION_STATUS["initialized"] else {}
    checkpoints = data_loader.get_available_checkpoints() if INITIALIZATION_STATUS["initialized"] else []
...
async def enhanced_batch_search(
    ...
    try:
        ...
        # Handle results based on return mode
        if all_results:
            logger.info(f"✅ Enhanced batch search completed - {len(all_results)} results")
            
            if return_session:
                # Store results in session and return session ID for batch viewer
                ...
                logger.info(f"📊 Batch results stored in session: {session_id}")
                ...
                return {
                    ...
                    "total_skus": len(sku_list),
                    "unique_images": len(sku_groups),
                    "message": "Batch processing completed. Results available in batch viewer."
                }
            else:
...
@app.get("/api/image/{filename_root}")
async def get_image_url(filename_root: str):
    """Get image URL for a given filename_root"""
    try:
        image_path = get_image_path(filename_root)
        if image_path:
        ...
        logger.error(f"Error getting image for {filename_root}: {e}")
        return {"image_url": None}
...
async def chrome_devtools_manifest():
    """Chrome DevTools manifest for remote debugging"""
    return {
        "name": "Hybrid Product Search Engine",
        "version": "1.0.0",
        "type": "node"
    }

@app.get("/api/batch-results/{session_id}")
async def get_batch_results_api(session_id: str):
    """Get batch results for the batch viewer"""
    try:
        session_data = get_batch_results(session_id)
        if not session_data:
            return JSONResponse(
                status_code=404,
                content={"error": "Batch session not found"}
            )
        
        # Group results by input SKU for easier navigation
        results_by_sku = {}
        for result in session_data['results']:
            input_sku = result.get('Input_SKU', 'Unknown')
            if input_sku not in results_by_sku:
                results_by_sku[input_sku] = []
            results_by_sku[input_sku].append(result)
        
        return {
            "results": results_by_sku,
            "metadata": session_data['metadata'],
            "total_skus": len(results_by_sku),
            "total_results": len(session_data['results'])
        }
        ...
        return JSONResponse(
            status_code=500,
            content={"error": f"Failed to retrieve batch results: {str(e)}"}
        )

@app.post("/api/upload-excel-results")
async def upload_excel_results(file: UploadFile = File(...)):
    """Upload Excel batch results and convert them for batch viewer"""
    try:
        logger.info(f"📤 Excel results upload request received")
        logger.info(f"📁 File: {file.filename}")
        ...
        # Convert DataFrame to batch results format
        results = []
        ...
        store_batch_results(session_id, results, metadata)
        
        logger.info(f"✅ Excel results uploaded and converted - {len(results)} results")
        
        return {
            "session_id": session_id,
            "total_results": len(results),
            "total_skus": len(df['Input_SKU'].unique()) if 'Input_SKU' in df.columns else len(results),
            "message": f"Successfully uploaded {len(results)} results from {file.filename}"
        }
