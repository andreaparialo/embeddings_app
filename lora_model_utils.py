...
def get_available_lora_models(finetuning_dir: str = "../finetuning/output_swift_balanced") -> List[Dict[str, str]]:
    """
    ...
    for model_dir in sorted(finetuning_output_dir.iterdir(), key=lambda x: x.name, reverse=True):
        if model_dir.is_dir() and model_dir.name.startswith("v"):
            # Find the latest checkpoint directory (highest step number)
            checkpoint_dirs = [d for d in model_dir.iterdir() if d.is_dir() and d.name.startswith("checkpoint-")]
            
            if checkpoint_dirs:
                # Sort by step number and take the latest (highest)
                checkpoint_dir = max(checkpoint_dirs, key=lambda x: int(x.name.split('-')[1]))
                ...
                if args_file.exists():
                    try:
                        ...
                        model_info['display_name'] = f"{model_info['version']} | Step:{model_info['checkpoint_step']} | {model_info['timestamp']} | LR:{model_info['learning_rate']} | Rank:{model_info['lora_rank']}"
...
def get_latest_lora_model(finetuning_dir: str = "../finetuning/output_swift_balanced") -> Optional[str]:
    """
...
def find_lora_model_by_version(version: str, finetuning_dir: str = "../finetuning/output_swift_balanced") -> Optional[str]:
    """
...
def list_lora_models_summary(finetuning_dir: str = "../finetuning/output_swift_balanced") -> None:
    """
...
def get_model_comparison_info(models: List[str], finetuning_dir: str = "../finetuning/output_swift_balanced") -> List[Dict]:
    """
